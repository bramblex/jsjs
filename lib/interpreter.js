...
var Scope = /** @class */ (function () {
    ...
    Scope.prototype.$declar = function (kind, name, value) {
        var _this = this;
        return ({
            'var': function () { return _this.$var(name, value); },
            'let': function () { return _this.$let(name, value); },
            'const': function () { return _this.$const(name, value); }
        })[kind]();
    };
    return Scope;
}());
...
function default_1(code, append_api) {
    var scope = new scope_1.default('block');
    for (var _i = 0, _a = Object.getOwnPropertyNames(append_api); _i < _a.length; _i++) {
        var name_1 = _a[_i];
        scope.$const(name_1, append_api[name_1]);
    }
...
var evaluate_map = {
    ...
    BlockStatement: function (block, scope) {
        var new_scope = scope.invasived ? scope : new scope_1.default('block', scope);
    ...
    SwitchStatement: function (node, scope) {
        ...
        var new_scope = new scope_1.default('switch', scope);
        var matched = false;
    ...
    TryStatement: function (node, scope) {
        try {
            ...
            if (node.handler) {
                ...
                var new_scope = new scope_1.default('block', scope);
    ...
    WhileStatement: function (node, scope) {
        while (evaluate(node.test, scope)) {
            var new_scope = new scope_1.default('loop', scope);
    ...
    ForStatement: function (node, scope) {
        for (var new_scope = new scope_1.default('loop', scope), init_val = node.init ? evaluate(node.init, new_scope) : null; node.test ? evaluate(node.test, new_scope) : true; node.update ? evaluate(node.update, new_scope) : void (0)) {
            var result = evaluate(node.body, new_scope);
            if (result === BREAK_SINGAL) {
                break;
            }
    ...
    FunctionDeclaration: function (node, scope) {
        var new_scope = new scope_1.default('function', scope);
        new_scope.invasived = true;
        var func = {
            func: function () {
                var args = [];
    ...
    VariableDeclaration: function (node, scope) {
        ...
        for (var _i = 0, _a = node.declarations; _i < _a.length; _i++) {
            ...
            scope.$declar(kind, name_1, value);
        }
    },
            ...
            'delete': function (v) {
                ...
                if (node.argument.type === 'MemberExpression') {
                    ...
                    if (computed) {
                        ...
                        delete evaluate(object, scope)[property.name];
                    }
                }
                else {
                    throw 'delete 后面只能接 member expr';
                }
            }
    ...
    CallExpression: function (node, scope) {
        ...
        return func.apply(_this, node.arguments.map(function (arg) { return evaluate(arg, scope); }));
    },
    NewExpression: function (node, scope) {
        var _class = evaluate(node.callee, scope);
        return new (_class.bind.apply(_class, node.arguments.map(function (arg) { return evaluate(arg, scope); })))();
    },
    ...
    Property: function (node, scope) { throw '这里如果被执行了那也是错的...'; },
